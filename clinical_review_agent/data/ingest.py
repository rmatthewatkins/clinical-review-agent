...
def _parse_note(r: dict) -> tuple:
    """Parse a Note resource. This is a custom (non-FHIR) resourceType because
    real-world FHIR R4 doesn't carry note narrative — that travels via C-CDA.
    MIMIC-III hands us raw text via mimiciii_notes.noteevents, which we wrap
    in a minimal envelope that stays consistent with the other parsers.
    """
    return (
        r["id"],
        ref(r.get("subject", {}).get("reference")),
...
def run_ingest(source: FhirSource | str) -> None:
    ...
    insert_order = [
        ...
        "observations", "procedures", "care_plans", "notes",
    ]
